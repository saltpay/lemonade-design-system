 ...
 *
 * This follows teya's canonical screenshot convention: authors stack Light/Dark
 * via `@Preview(uiMode = …)` (see [com.teya.lemonade.LemonadePreview]) and the
 * tester captures one image per preview, picking the theme from that `uiMode`.
 *
    ...
    override fun test(testParameter: AndroidPreviewJUnit4TestParameter) {
        ...
        captureRoboImage(
            ...
            LemonadeTheme(colors = colors) {
                // Paint the themed surface behind the preview. Previews no longer
                // set `showBackground`, so without this the capture surface is
                // transparent and dark-theme content (light text) would be
                // invisible once the PNG is flattened.
