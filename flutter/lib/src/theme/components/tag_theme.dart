...
class LemonadeTagTheme {
  ...
  const LemonadeTagTheme({required this.iconSize});
  ...
  factory LemonadeTagTheme.from(LemonadeTokens tokens) {
    return LemonadeTagTheme(iconSize: tokens.sizes.size400);
  }
  ...
  factory LemonadeTagTheme.lerp(
    ...
    return LemonadeTagTheme(iconSize: lerpDouble(a.iconSize, b.iconSize, t)!);
  }
  ...
  LemonadeTagTheme copyWith({double? iconSize}) {
    return LemonadeTagTheme(iconSize: iconSize ?? this.iconSize);
  }

  /// Merges this theme with another theme.
  LemonadeTagTheme mergeWith(LemonadeTagTheme? other) {
    if (other == null) return this;
    return copyWith(iconSize: other.iconSize);
  }
